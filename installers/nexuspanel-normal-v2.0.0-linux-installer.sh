...
RELEASE_TAG="${NEXUSPANEL_RELEASE_TAG:-${EDITION}-v2.0.0}"
NODE_PATH=""

need_root() {
  if [ "$(id -u)" -ne 0 ]; then
    echo "Run as root: sudo bash installers/nexuspanel-normal-v2.0.0-linux-installer.sh"
    exit 1
  fi
}
...
main() {
  echo "NexusPanel Linux Installer v2.0.0"
  echo
  need_root
  require_service_user
  install_packages
