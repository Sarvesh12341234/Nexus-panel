...
- Its bounded hybrid agent uses a 299,598-parameter hashed neural ranker, live VPS/server telemetry, a cause-and-action knowledge graph, and a persistent reinforcement history while keeping its estimated resident model state below 23 MB.
...
- AI terminal tools use a hidden allowlisted command channel for VPS probes such as disk, memory, systemd, ports, and Java runtime checks. Each probe is timeout-capped, redacted, audited, and protected by an internal hash key.
...
- AI Full Access Mode is available from the Fixed section for owners: unlock it with the owner password for a limited window, queue any shell command, then approve each command before it runs. Commands are risk-labeled, timeout-capped, redacted, and audited.
- Live Agent mode continuously checks console/file repair signals during adaptive maintenance, learns from incidents, applies safe offline repairs, and queues risky shell work for owner approval.
