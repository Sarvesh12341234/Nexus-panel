...
const { getBackupInterval, setBackupInterval, formatBackupInterval, getNextBackupTime } = require('./backup-utils');
...
app.get('/api/servers/:id', requireAuth, (req, res) => {
  ...
  const interval = getBackupInterval(server.id);
  const nextBackup = getNextBackupTime(server.id);
  
  res.json({
    ...serverPayload(server, req),
    backup_interval_hours: interval.hours,
    backup_interval_minutes: interval.minutes,
    backup_interval_formatted: formatBackupInterval(interval.hours, interval.minutes),
