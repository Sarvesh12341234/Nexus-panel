...
function startServer(server, software) {
  ...
  // FIX: Use server_path if it exists, otherwise use ensureServerDirs
  const root = server.server_path || ensureServerDirs(server);
  
  // Log where we're starting from for debugging
  ...
  // IMPORTANT: spawn with the correct working directory
  const child = spawn(wrapped.command, wrapped.args, spawnOptions({
    cwd: root,  // This must be the server directory
